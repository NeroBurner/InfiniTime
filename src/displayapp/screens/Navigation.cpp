...
Navigation::Navigation(Pinetime::Applications::DisplayApp* app, Pinetime::Controllers::NavigationService& nav)
  ...
  imgFlag = lv_label_create(lv_scr_act());
  ...
  lv_obj_set_style_bg_color(barProgress, lv_color_hex(0x222222), LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_set_style_bg_color(barProgress, lv_palette_main(LV_PALETTE_ORANGE), LV_PART_INDICATOR | LV_STATE_DEFAULT);
  lv_obj_set_style_anim_time(barProgress, 500, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void Navigation::Refresh() {
  ...
  if (progress != navService.getProgress()) {
    ...
    if (progress > 90) {
      lv_obj_set_style_bg_color(barProgress, lv_palette_main(LV_PALETTE_RED), LV_PART_INDICATOR | LV_STATE_DEFAULT);
    } else {
      lv_obj_set_style_bg_color(barProgress, lv_palette_main(LV_PALETTE_ORANGE), LV_PART_INDICATOR | LV_STATE_DEFAULT);
    }
  }
